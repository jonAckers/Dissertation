...
\begin{table}[h!]
    ...
    % \begin{tabular}{|>{\centering\arraybackslash}m{2.67cm}|>{\centering\arraybackslash}m{2.1cm}|m{14cm}|}
        \resizebox{\textwidth}{!}{%
    \begin{tabular}{|>{\centering\arraybackslash}m{2.67cm}|>{\centering\arraybackslash}m{2.1cm}|m{12cm}|}
        \hline
        \textrm{\textbf{Requirement}} & \textrm{\textbf{Achieved?}} & \textrm{\textbf{Justification}} \\
        \hline \hline
        \texttt{A1} & \cmark & \textrm{The project contains a client-server application that allows videos to be homomorphically encrypted and transmitted across a network.} \\
        \hline
        \texttt{A2} & \cmark & \textrm{The project contains several algorithms that are able to extract moving objects from homomorphically encrypted videos.} \\
        \hline
        ...
        \hline
        \texttt{B2} & \xmark &  \textrm{Due to time constraints, an independent investigation into the security of HE schemes could not be completed. However, only well-established, trusted schemes were considered; hence CKKS was selected over less secure schemes.} \\
        \hline
...
\section{Evaluating Real-World Applicability}
\indent \indent
